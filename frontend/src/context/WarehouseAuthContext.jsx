...
export const WarehouseAuthProvider = ({ children }) => {
  ...
  const clearStoredAuth = () => {
    ...
    localStorage.removeItem('warehouse_details')
  }
  ...
  const checkWarehouseAuth = async () => {
    try {
      ...
      const cachedDetails = localStorage.getItem('warehouse_details')
      if (cachedDetails) {
        try {
      ...
      setAuthToken(token)
      localStorage.setItem('warehouse_details', JSON.stringify(data.warehouse))
    } catch (err) {
