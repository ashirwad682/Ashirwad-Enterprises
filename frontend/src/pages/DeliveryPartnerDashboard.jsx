...
const ORDERS_CACHE_KEY = 'delivery_orders_cache'
function saveOrdersToCache(orders) {
  try { sessionStorage.setItem(ORDERS_CACHE_KEY, JSON.stringify(orders)) } catch (_) {}
}
...
export default function DeliveryPartnerDashboard() {
  ...
  const [orders, setOrders] = useState(() => loadOrdersFromCache())
  ...
  // Sort priority:
  //   1. Express orders first, then Standard
  //   2. Within Express: earliest expected delivery deadline first (most urgent at top)
  //   3. Within Standard: earliest expected delivery deadline first
  const sortedActiveOrders = useMemo(() => {
    const isExpress = (o) =>
      (o.shipping_method || '').toLowerCase() === 'express' ||
      Number(o.shipping_fee || 0) > 0

    const getDeliveryTs = (o) => {
      const info = getExpectedDelivery(o)
      return info?.date ? info.date.getTime() : Infinity
    }

    return [...activeOrders].sort((a, b) => {
      const aEx = isExpress(a) ? 0 : 1
      const bEx = isExpress(b) ? 0 : 1
      // Different groups — Express wins
      if (aEx !== bEx) return aEx - bEx
      // Same group — sort by delivery deadline ascending (soonest first)
      return getDeliveryTs(a) - getDeliveryTs(b)
    })
  }, [activeOrders])
  ...
  const loadAssignedOrders = async () => {
    ...
    try {
      ...
      saveOrdersToCache(enrichedOrders)
      setError(null)
    } catch (err) {
  ...
  const renderVerificationNudge = () => {
    if (!profileData || activeTab !== 'dashboard') return null

    const partner = profileData.partner
    const documents = profileData.documents || []

    const photoMissing = !partner?.profile_photo_url
    const uploadedTypes = new Set(documents.map(d => d.document_type))
    const docsMissing = !uploadedTypes.has('aadhaar') || !uploadedTypes.has('driving_license') || !uploadedTypes.has('vehicle_rc')
    const hasRejected = documents.some(d => d.status === 'rejected')
    const allApproved = partner?.verification_status === 'approved'
    ...
    if (hasRejected) {
      ...
      message = 'One or more of your uploaded documents were rejected by the administrator. Please update them immediately.'
      type = 'error'
      actionText = 'Fix Documents'
    } else if (photoMissing || docsMissing) {
      title = '⚠️ Complete Your Profile Verification'
      message = 'Please capture a live photo and upload your Aadhaar, Driving License, and Vehicle RC to verify your identity.'
      type = 'warning'
      actionText = 'Upload Documents'
    } else if (partner?.verification_status === 'pending' || partner?.verification_status === 'under_review') {
      title = '⏳ Documents Under Review'
      message = 'Your documents have been submitted and are currently under review by the administrator. We will notify you once verified.'
      type = 'info'
    ...
    const bgColors = {
      ...
      info: 'linear-gradient(135deg, #f0f9ff 0%, #e0f2fe 100%)'
    }
    ...
    return (
      ...
      >
        <div style={{ flex: 1, minWidth: 260 }}>
          <div style={{ fontSize: 15, fontWeight: 800, color: textColors[type], marginBottom: 4 }}>
            {title}
          </div>
          <div style={{ fontSize: 13, color: textColors[type], opacity: 0.9, lineHeight: 1.5 }}>
            {message}
          </div>
        </div>
        <button
        ...
        >
          ...
          <div style={{ position: 'relative', display: 'flex', justifyContent: 'space-between', alignItems: isMobile ? 'stretch' : 'center', flexDirection: isMobile ? 'column' : 'row', gap: isMobile ? 16 : 20 }}>
            ...
            <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: isMobile ? 'wrap' : 'nowrap' }}>
              ...
              <button
                onClick={() => handleTabChange(activeTab === 'dashboard' ? 'profile' : 'dashboard')}
                style={{
                  padding: '10px 16px',
                  background: activeTab === 'profile' ? 'linear-gradient(135deg, #4f46e5 0%, #3730a3 100%)' : '#fff',
                  color: activeTab === 'profile' ? '#fff' : '#111827',
                  borderRadius: 12,
                  border: activeTab === 'profile' ? 'none' : '1px solid #e2e8f0',
                  fontWeight: 700,
                  cursor: 'pointer',
                  fontSize: 14,
                  ...
                  alignItems: 'center',
                  gap: 8,
                  boxShadow: activeTab === 'profile' ? '0 8px 20px rgba(79, 70, 229, 0.25)' : '0 2px 8px rgba(15, 23, 42, 0.05)'
                }}
              >
                {activeTab === 'profile' ? '📊 View Dashboard' : '👤 My Profile'}
              </button>
        ...
        {activeTab === 'dashboard' ? (
          <>
            ...
            <div>
              ...
              {/* Show cached orders while loading to prevent flicker; replace with fresh data when ready */}
              {loading && activeOrders.length === 0 ? (
                <div style={{ display: 'grid', gap: 16 }}>
                  {[1, 2, 3].map(i => (
                    <div key={i} style={{
                      padding: 20, borderRadius: 12, border: '1px solid #e5e7eb',
                      background: '#fafafa', animation: 'pulse 1.5s ease-in-out infinite'
                    }}>
                      <div style={{ height: 16, width: '40%', background: '#e2e8f0', borderRadius: 6, marginBottom: 12 }} />
          ...
          <DeliveryPartnerProfile deliveryPartnerId={deliveryPartner?.id} />
        )}
  ...
  return (
    <motion.div
      ...
      style={{
        ...
        borderRadius: 12,
        border: isExpress ? '1.5px solid #f59e0b' : '1px solid #e5e7eb',
        borderLeft: isExpress ? '4px solid #f59e0b' : undefined,
        background: isExpress ? 'linear-gradient(135deg, #fffbeb 0%, #ffffff 60%)' : '#ffffff',
        boxShadow: isExpress
          ? '0 0 0 3px rgba(245, 158, 11, 0.10), 0 4px 16px rgba(245,158,11,0.08)'
          : ['dispatched', 'out_for_delivery'].includes(normalizedStatus)
            ? '0 0 0 3px rgba(139, 92, 246, 0.12)'
            : 'none'
      }}
    >
      {/* Express delivery priority banner */}
      {isExpress && (
        <div style={{
          display: 'inline-flex',
          alignItems: 'center',
