...
import { fetchOffers, fetchProducts, fetchUserOrders, fetchNotifications } from '../api/client'
...
export default function UserDashboard() {
  ...
  const [user, setUser] = useState(cachedUserProfile)
  const [loading, setLoading] = useState(!cachedUserProfile)
  const [canOrder, setCanOrder] = useState(cachedUserProfile ? Boolean(cachedUserProfile.is_verified) : false)
  const [logoutError, setLogoutError] = useState(null)
  const [refreshing, setRefreshing] = useState(false)
  const [unreadNotifCount, setUnreadNotifCount] = useState(0)

  const loadUser = useCallback(async (silent = false) => {
    const isSilent = silent || Boolean(cachedUserProfile)
    if (isSilent) {
      setRefreshing(true)
    } else {
      setLoading(true)
    ...
    try {
      ...
      // Trigger background prefetching to warm up all caches instantly
      try {
        fetchOffers().catch(() => {})
        fetchProducts().catch(() => {})
        fetchUserOrders(profile.id).catch(() => {})
        fetchNotifications(profile.id).catch(() => {})
      } catch (_) {}
  ...
  const isHomeView = normalisedPath === '/dashboard' || normalisedPath.endsWith('/home')
  ...
  const isOrderSuccessView = normalisedPath.includes('/order-success');
  const showBareSection = isCheckoutView || isCartView || isHomeView || isOrderSuccessView
          ...
          <main style={{ flex: 1, display: 'grid', gap: isCheckoutView || isCartView ? 12 : 20 }}>
            ...
            >
              <Routes>
                ...
                <Route path="track" element={<TrackOrder userId={user?.id} />} />
                <Route path="track-order" element={<TrackOrder userId={user?.id} />} />
