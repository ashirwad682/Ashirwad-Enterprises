...
router.get('/track/:reference', async (req, res) => {
  try {
    ...
    if (uuidRegex.test(reference)) {
      const { data, error } = await supabase
        .from('orders')
        .select('id, status, delivery_status, created_at, updated_at, delivered_at, total_amount, payment_method, cod_amount_received, shipping_method, shipping_fee, pickup_order')
        .eq('id', reference)
        .limit(1)
      orders = data || []
      queryError = error
    } else {
      // Return empty if not a valid UUID format, as Postgres cannot perform partial matches on UUIDs directly
      orders = []
    }
    ...
    res.json({
      ...
      order: {
        ...
        pickup_order: order.pickup_order
      },
      timeline
    })
  } catch (err) {
  ...
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert({
        ...
        assigned_area: String(assignedArea).trim(),
        status: 'active'
      })
      .select()
      .single()
...
router.get('/admin/delivery-partners', async (req, res) => {
  try {
    ...
    const { data: partners, error: partnerErr } = await supabase
      .from('delivery_partners')
      .select('id, delivery_partner_id, name, email, mobile_number, assigned_area, status, verification_status, overall_approval_status, created_at')
      .order('created_at', { ascending: false })

    if (partnerErr) throw partnerErr

    // Fetch document counts grouped by partner
    const { data: docs, error: docsErr } = await supabase
      .from('delivery_partner_documents')
      .select('delivery_partner_id, document_type, status')

    if (docsErr) throw docsErr

    const partnerDocsMap = {}
    if (docs) {
      docs.forEach(doc => {
        if (!partnerDocsMap[doc.delivery_partner_id]) {
          partnerDocsMap[doc.delivery_partner_id] = []
        }
        partnerDocsMap[doc.delivery_partner_id].push(doc)
      })
    }

    const result = (partners || []).map(p => {
      const pDocs = partnerDocsMap[p.id] || []
      const pendingCount = pDocs.filter(d => d.status === 'pending').length
      const approvedCount = pDocs.filter(d => d.status === 'approved').length
      const rejectedCount = pDocs.filter(d => d.status === 'rejected').length
...
router.patch('/admin/orders/:orderId/assign', async (req, res) => {
  try {
    ...
    const { data: dp, error: dpErr } = await supabase
      .from('delivery_partners')
      .select('id, delivery_partner_id, status, name, mobile_number')
    ...
    // Create user notification for assignment
    try {
      const paymentMethod = (order.payment_method || '').toUpperCase() === 'COD' ? 'COD (Unpaid)' : 'Prepaid'
      const formattedOrderId = order.id.startsWith('OD-') ? order.id : `#${order.id.slice(0, 8).toUpperCase()}`
      ...
      const lines = [
        ...
        `Your order (Order ID: ${formattedOrderId}) has been approved and assigned to the delivery partner ${dp.name || 'N/A'}.`,
        `Amount: ${formattedAmount}`,
        `Payment Method: ${paymentMethod}`
      ]

      if (dp.mobile_number) {
        lines.push(`Delivery Partner Phone: ${dp.mobile_number}`)
      }

      await supabase
        .from('notifications')
        .insert([{ user_id: order.user_id, message: lines.join('\n') }])
    } catch (notifErr) {
      console.warn('Failed to insert assignment notification in admin route:', notifErr?.message || notifErr)
    }
...
// Multer and file upload config for profile verification
const upload = multer({
...
const runMulterMiddleware = (req, res, fn) => {
...
// Helper to sign document URLs for private bucket
const signDocumentUrls = async (docs) => {
  if (!docs || !Array.isArray(docs)) return []
  return Promise.all(
    docs.map(async (doc) => {
      if (doc.storage_path) {
        try {
          const bucket = doc.document_type === 'profile_photo' ? 'delivery-partner-profile-photos' : 'delivery-partner-documents'
          if (bucket === 'delivery-partner-profile-photos') {
            const { data } = supabase.storage.from(bucket).getPublicUrl(doc.storage_path)
            return { ...doc, file_url: data?.publicUrl || doc.file_url }
          } else {
          ...
          console.warn('Failed to sign URL for doc:', doc.storage_path, e.message)
        }
      }
      return doc
    })
 ...
 * Get own full profile + documents + status
 */
router.get('/profile', verifyDeliveryPartner, async (req, res) => {
  try {
    const { data: partner, error: partnerErr } = await supabase
      .from('delivery_partners')
      .select('id, delivery_partner_id, name, email, mobile_number, assigned_area, status, address_line, address_city, address_state, address_pincode, vehicle_type, vehicle_number, profile_photo_url, profile_photo_locked, profile_details_locked, verification_status, overall_approval_status, created_at')
      .eq('id', req.dpId)
      .single()

    if (partnerErr || !partner) {
      return res.status(404).json({ error: 'Delivery partner profile not found' })
    }

    const { data: docs, error: docsErr } = await supabase
      .from('delivery_partner_documents')
      .select('*')
      .eq('delivery_partner_id', req.dpId)

    const signedDocs = await signDocumentUrls(docs || [])
 ...
 * Update profile details (address and vehicle info) if not locked
 */
router.put('/profile', verifyDeliveryPartner, async (req, res) => {
  try {
    const { address_line, address_city, address_state, address_pincode, vehicle_type, vehicle_number } = req.body

    const { data: partner, error: getErr } = await supabase
      .from('delivery_partners')
      .select('profile_details_locked')
      .eq('id', req.dpId)
      .single()
    ...
    if (partner.profile_details_locked) {
      return res.status(403).json({ error: 'Profile details are locked and cannot be edited.' })
    }
...
/**
 * PUT /api/delivery/profile/phone
 * Update phone number (only editable field)
 */
router.put('/profile/phone', verifyDeliveryPartner, async (req, res) => {
  try {
    const { mobile_number } = req.body

    if (!mobile_number || !/^\+?\d{8,15}$/.test(mobile_number.trim())) {
...
/**
 * POST /api/delivery/profile/photo
 * Upload live-captured profile photo
 */
router.post('/profile/photo', verifyDeliveryPartner, async (req, res) => {
  try {
    await runMulterMiddleware(req, res, upload.single('photo'))
    ...
    if (source !== 'camera' && source !== 'live') {
      return res.status(403).json({ error: 'Only live camera capture is allowed for profile photos.' })
    }

    const { data: partner, error: partnerErr } = await supabase
      .from('delivery_partners')
      .select('profile_photo_locked')
      .eq('id', req.dpId)
      .single()
    ...
    if (partner.profile_photo_locked) {
      return res.status(403).json({ error: 'Profile photo is locked and cannot be updated.' })
    }

    const extension = MIME_EXTENSION_MAP[req.file.mimetype] || 'jpg'
    const storagePath = `${req.dpId}/photo-${Date.now()}-${crypto.randomBytes(8).toString('hex')}.${extension}`

    const { error: storageError } = await supabase.storage
      .from('delivery-partner-profile-photos')
    ...
    // Update profile url in delivery_partners
    ...
    // Upsert into delivery_partner_documents
    const { error: docUpsertErr } = await supabase
      .from('delivery_partner_documents')
      .upsert({
        delivery_partner_id: req.dpId,
        document_type: 'profile_photo',
        file_name: req.file.originalname || 'profile_photo.jpg',
        ...
        updated_at: new Date().toISOString()
      }, { onConflict: 'delivery_partner_id,document_type' })

    if (docUpsertErr) throw docUpsertErr

    // Add audit log
    await supabase.from('delivery_partner_verification_audit').insert({
      delivery_partner_id: req.dpId,
      action: 'upload_photo',
      document_type: 'profile_photo',
      new_status: 'pending',
      performed_by: 'partner',
      notes: 'Live profile photo uploaded by delivery partner'
    })

    res.json({
      success: true,
      message: 'Profile photo uploaded and submitted for review',
      profile_photo_url: publicUrl
    })
  } catch (err) {
...
/**
 * POST /api/delivery/documents/:type
 * Upload a document (aadhaar, driving_license, vehicle_rc)
 */
router.post('/documents/:type', verifyDeliveryPartner, async (req, res) => {
  try {
    const { type } = req.params
    if (!['aadhaar', 'driving_license', 'vehicle_rc'].includes(type)) {
      return res.status(400).json({ error: 'Invalid document type' })
    }

    await runMulterMiddleware(req, res, upload.single('document'))

    if (!req.file) {
      return res.status(400).json({ error: 'Document file is required' })
    }

    // Check if locked (pending/approved status locked, re-upload allowed only for rejected/not uploaded)
    ...
    if (existing && (existing.status === 'approved' || existing.status === 'pending')) {
      return res.status(403).json({ error: 'Document is locked and cannot be updated unless rejected by Admin.' })
    }

    const extension = MIME_EXTENSION_MAP[req.file.mimetype] || 'pdf'
    const storagePath = `${req.dpId}/${type}-${Date.now()}-${crypto.randomBytes(8).toString('hex')}.${extension}`
    ...
    // Update overall partner verification status to under review
    await supabase
      .from('delivery_partners')
      .update({
        verification_status: 'under_review',
    ...
    await supabase.from('delivery_partner_verification_audit').insert({
      ...
      new_status: 'pending',
      performed_by: 'partner',
      notes: `Uploaded document of type ${type}`
    })

    res.json({
      success: true,
      message: 'Document uploaded and submitted for review successfully.'
    })
  } catch (err) {
 ...
 * Get all own documents with status
 */
router.get('/documents', verifyDeliveryPartner, async (req, res) => {
  try {
    ...
    const signedDocs = await signDocumentUrls(docs || [])
    res.json({ success: true, documents: signedDocs })
  } catch (err) {
...
// Duplicate route removed (GET /admin/delivery-partners now combined above for backward compatibility)

/**
 * GET /api/admin/delivery-partners/:id
 * Get full partner profile + all documents (admin only)
 */
router.get('/admin/delivery-partners/:id', verifyAdminApiKey, async (req, res) => {
  try {
    const { id } = req.params
...
/**
 * POST /api/admin/delivery-partners/:id/documents/:docId/approve
 * Approve specific document (admin only)
 */
router.post('/admin/delivery-partners/:id/documents/:docId/approve', verifyAdminApiKey, async (req, res) => {
  try {
    ...
    await supabase.from('delivery_partner_verification_audit').insert({
      ...
      new_status: 'approved',
      performed_by: 'admin',
      notes: `Document of type '${doc.document_type}' approved by Admin`
    })
...
/**
 * POST /api/admin/delivery-partners/:id/documents/:docId/reject
 * Reject specific document with reason (admin only)
 */
router.post('/admin/delivery-partners/:id/documents/:docId/reject', verifyAdminApiKey, async (req, res) => {
  try {
    const { id, docId } = req.params
    ...
    // Update overall partner verification status to rejected
    await supabase
      .from('delivery_partners')
      .update({
    ...
    await supabase.from('delivery_partner_verification_audit').insert({
      ...
      rejection_reason: rejection_reason.trim(),
      performed_by: 'admin',
      notes: `Document '${doc.document_type}' rejected: ${rejection_reason.trim()}`
    })
 ...
 * Full profile approval (admin only)
 */
router.post('/admin/delivery-partners/:id/approve', verifyAdminApiKey, async (req, res) => {
  try {
    const { id } = req.params
    ...
    await supabase.from('delivery_partner_verification_audit').insert({
      ...
      new_status: 'approved',
      performed_by: 'admin',
      notes: 'Overall delivery partner profile approved and locked by Admin'
    })

    res.json({ success: true, message: 'Delivery partner profile fully approved.' })
  } catch (err) {
    console.error('Admin approve profile error:', err)
    res.status(500).json({ error: 'Failed to approve partner profile' })
  }
})

/**
 * POST /api/admin/delivery-partners/:id/reject
 * Full profile rejection with reason (admin only)
 */
router.post('/admin/delivery-partners/:id/reject', verifyAdminApiKey, async (req, res) => {
  try {
    const { id } = req.params
    ...
    await supabase.from('delivery_partner_verification_audit').insert({
      ...
      new_status: 'rejected',
      rejection_reason: reason || 'Profile details rejected',
      performed_by: 'admin',
      notes: `Overall profile rejected: ${reason || 'No specific reason'}`
    })

    res.json({ success: true, message: 'Delivery partner profile rejected.' })
  } catch (err) {
    console.error('Admin reject profile error:', err)
    res.status(500).json({ error: 'Failed to reject partner profile' })
  }
})

/**
 * GET /api/admin/delivery-partners/:id/audit
 * Get verification audit trail (admin only)
 */
router.get('/admin/delivery-partners/:id/audit', verifyAdminApiKey, async (req, res) => {
  try {
    const { id } = req.params

    const { data: logs, error } = await supabase
      .from('delivery_partner_verification_audit')
      .select('*')
      .eq('delivery_partner_id', id)
...
/**
 * GET /api/delivery/bank-account
 * Retrieve delivery partner's bank account details
 */
router.get('/bank-account', verifyDeliveryPartner, async (req, res) => {
  try {
    ...
    console.error('Failed to fetch delivery partner bank account:', err)
    res.status(500).json({ error: 'Failed to retrieve bank account details' })
  }
})

/**
 * POST /api/delivery/bank-account
 * Add or update delivery partner's bank account details
 */
router.post('/bank-account', verifyDeliveryPartner, async (req, res) => {
  ...
  try {
    ...
    console.error('Failed to save delivery partner bank account:', err)
    res.status(500).json({ error: 'Failed to save bank account details' })
  }
})

/**
 * DELETE /api/delivery/bank-account
 * Delete pending update request or unapproved bank details
 */
router.delete('/bank-account', verifyDeliveryPartner, async (req, res) => {
  try {
    const { data: existingRows, error: fetchError } = await supabase
      .from('bank_accounts')
      .select('*')
    ...
    console.error('Failed to delete delivery partner bank account:', err)
