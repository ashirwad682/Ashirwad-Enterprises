...
router.get('/dashboard-blocking', async (req, res) => {
  try {
    ...
    // Check if current time is after block time in India Time (IST)
    const now = new Date()
    const indiaTime = new Date(now.getTime() + (5.5 * 60 * 60 * 1000))
    const paddedHours = String(indiaTime.getUTCHours()).padStart(2, '0')
    const paddedMinutes = String(indiaTime.getUTCMinutes()).padStart(2, '0')
    const normalizedIndiaTime = `${paddedHours}:${paddedMinutes}`
    
    // Normalize and compare
    const normalizedBlockTime = data.block_time.split(':').slice(0, 2).map(s => s.padStart(2, '0')).join(':')
    
    const isBlocked = data.is_enabled && normalizedIndiaTime >= normalizedBlockTime

    // Construct nextBlockTime in Indian Time
    const y = indiaTime.getUTCFullYear()
    const m = String(indiaTime.getUTCMonth() + 1).padStart(2, '0')
    const d = String(indiaTime.getUTCDate()).padStart(2, '0')
    
    const [blockHours, blockMinutes] = data.block_time.split(':').map(Number)
    const paddedH = String(blockHours).padStart(2, '0')
    const paddedM = String(blockMinutes).padStart(2, '0')
    const indiaIsoStr = `${y}-${m}-${d}T${paddedH}:${paddedM}:00`
    
    // Since India is UTC+5:30
    const blockTime = new Date(`${indiaIsoStr}+05:30`)
...
router.post('/check-dashboard-access', async (req, res) => {
  try {
    ...
    // Check if within block time in India Time (IST)
    const now = new Date()
    const indiaTime = new Date(now.getTime() + (5.5 * 60 * 60 * 1000))
    const paddedHours = String(indiaTime.getUTCHours()).padStart(2, '0')
    const paddedMinutes = String(indiaTime.getUTCMinutes()).padStart(2, '0')
    ...
    // Normalize and compare
    const normalizedBlockTime = blockConfig.block_time.split(':').slice(0, 2).map(s => s.padStart(2, '0')).join(':')
    
    const isBlocked = blockConfig.is_enabled && normalizedIndiaTime >= normalizedBlockTime
...
router.post('/admin/generate-access-key', async (req, res) => {
  try {
    ...
    res.json({
      ...
      accessKey: accessKey,
      partnerId: partnerId,
      partnerName: partner.name || partner.mobile_number,
  ...
  try {
    ...
    const { data: keys, error } = await req.supabase
      ...
      .select(`
        ...
        used_at,
        generated_at,
        delivery_partners(id, name, mobile_number, email)
      `)
    ...
    res.json({
      ...
      keys: activeKeys.map(key => ({
        ...
        partnerName: key.delivery_partners?.name || key.delivery_partners?.mobile_number || 'Unknown',
        partnerPhone: key.delivery_partners?.mobile_number,
