...
async function runMigration() {
  console.log('🚀 Executing Salaries and Refunds Database Migration...\n')
  const sqlPath = join(__dirname, 'ADD_SALARIES_AND_REFUNDS.sql')
  const sql = readFileSync(sqlPath, 'utf8')

  try {
    console.log('Trying rpc("sql", { query: sql })...')
    const { data: data1, error: error1 } = await supabase.rpc('sql', { query: sql })
    if (!error1) {
      console.log('✅ Migration executed successfully via sql!')
      console.log(data1)
      return
    }
    console.warn('⚠️ sql failed:', error1.message || error1)

    console.log('Trying direct HTTP call to /rest/v1/rpc/sql...')
    const response2 = await fetch(`${SUPABASE_URL}/rest/v1/rpc/sql`, {
      method: 'POST',
      headers: {
    ...
    console.error('❌ HTTP RPC /sql failed:', response2.status, response2Text)
    process.exit(1)
  } catch (err) {
